/*
    Функция для умножения двух децимал. Принимает 2 числа децимал, которые надо
   умножить и указатель для записи результата.
*/
...
int shift_sum(int shift1, int shift2) {
  int sum = 0;
  if (shift1 > 0) sum += shift1;
  if (shift2 > 0) sum += shift2;
  return sum;
}

/*
    Умножаем первую строку на каждый элемент второго массива и складываем
   результат. При умножении на второй и далее элемент сдвигаем строку и
   дополняем её нулями. Пример: 421 * 79 = 33259 124
    * 97
    = 9873 +
      07492
...
/*
   Так как вся обработка строк идёт задом наперёд сначала в res записываются
   нули, соответствующие сдвигу при умножении в столбик. Затем со сдвигом на
   число j производится умножение остальной строки и запись результата. Пример:
   15 * 125 = 1875 51
   * 521
   = 57 +
     003 +
...
int size_check(char *dec, int sign) {
  ...
  if (offset > 0) {
    // Сначала копируем цифры до запятой. Если перед точкой только 0, то
    // пропускаем
    int i = 0;
    for (; i < offset; i++) {
      if (dec[0] == '0') break;
  ...
  // Если общее количество значащих цифр больше 29 или количество знаков после
  // запятой больше 28 Если надо будет добавить округление числа, не
  // помещающееся в мантиссу, то здесь
